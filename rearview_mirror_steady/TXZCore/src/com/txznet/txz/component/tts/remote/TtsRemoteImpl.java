...
public class TtsRemoteImpl implements ITts {
	...
	private static final int DEFAULT_TIMEOUT = 5000; // 默认超时时长
	...
	public static void setRemoteTtsService(String serviceName) {
		if (DebugCfg.DISABLE_REMOTE_TTS_TOOL && !TXZService.isTXZSelfService(mRemoteTtsService)) {
			return;
		}
	 ...
	 * 远程工具 tts 播报超时
	 */
	private static Runnable mTtsTimeoutRunnable = new Runnable() {
	...
	public int start(int iStream, String sText, ITtsCallback oRun) {
		...
		AppLogic.runOnBackGround(mTtsTimeoutRunnable, sText.length() * 500 + DEFAULT_TIMEOUT);
