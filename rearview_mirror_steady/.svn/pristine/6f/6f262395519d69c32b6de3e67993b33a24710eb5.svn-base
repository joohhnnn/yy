...
public class UpgradeFloatView extends LinearLayout {
    ...
    protected void onAttachedToWindow() {
        ...
        LogUtil.logd("UpgradeFloatView onAttachedToWindow");
        getViewTreeObserver().addOnPreDrawListener(new OnPreDrawListener() {
            @Override
            public boolean onPreDraw() {
                LogUtil.logd("UpgradeFloatView onPreDraw");
                ...
                try {
                    ...
                    if (cacheX < 0) {
                        cacheX = 0;
                    } else if (cacheX > mRootWidth - mWidth -10) {
                        cacheX = mRootWidth - mWidth -10;
                    }
                    if (cacheY < 26) {
                        cacheY = 26;
                    ...
                    LogUtil.logd("UpgradeFloatView onPreDraw finish x=" + mLp.x
    ...
    public void open() {
        ...
        mIsOpening = true;
        LogUtil.logd("UpgradeFloatView OPEN x=" + mLp.x + ", y=" + mLp.y + ", w="
            ...
            case MotionEvent.ACTION_MOVE:
                if (isTouchVoiceAssistant) {
                    ...
                    if (shouldMove) {
                        ...
                        if (mLp.x < 0) {
                            mLp.x = 0;
                        } else if (mLp.x > mRootWidth - mWidth - 10) {
                            mLp.x = mRootWidth - mWidth - 10;
                        }
                        if (mLp.y < 26) {
                            mLp.y = 26;
...
//		            playSoundEffect(SoundEffectConstants.CLICK);
                        LogUtil.logd("UpgradeFloatView doLaunch");
//		            LaunchManager.getInstance().launchWithRecord();
                    ...
                    if (mLp.x < mWidth / 2) {
                        mLp.x = 0;
                    } else if (mLp.x > mRootWidth - mWidth - 10 - mWidth / 2) {
                        mLp.x = mRootWidth - mWidth - 10;
                    }
                    if (mLp.y < mHeight / 2) {
                        mLp.y = 26;
                    ...
                    PointCache.getInstance(getContext()).setX(historyX);
                    PointCache.getInstance(getContext()).setY(historyY);
                    LogUtil.logd("UpgradeFloatView SET x=" + mLp.x + ", y=" + mLp.y
