...
public class AsrManager extends IModule {
	public static int ASR_CHAT_MODE_MAX_UNKNOW_COUNT = 1; // 聊天模式下最多支持几次识别不了后退出
	...
	public void showHelpTips(){
		LogUtil.logd("start show help...."+(isShowHelp&&isFirstRecord));
		Thread thread = new Thread(new Runnable() {
			@Override
			public void run() {
				try {
					LogUtil.logd("start show help  sleep....");
					Thread.sleep(2000);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				if(isShowHelp&&isFirstRecord){
					LogUtil.logd("start show help  WinManager....");
					if (WinManager.getInstance().canShowHelpTips() && WinHelpManager.getInstance().isNotNullHelpTips()) {
	...
	IAsrCallback mSysAsrCallBack = new IAsrCallback() {
		...
		public void onStart(AsrOption option) {
			...
			//没有网络的话先显示一个空的框，TODO 出现的时机和没有说话显示的帮助这种可能会有冲突
			if (!NetworkManager.getInstance().hasNet() && ProjectCfg.enableTypingEffect() ) {
		...
		public void onPartialResult(AsrOption option ,String partialResult) {
			...
			if (!ChoiceManager.getInstance().isSelecting() && ProjectCfg.enableTypingEffect()) {
				if (DebugCfg.TYPING_EFFECT_DEBUG) {
					LogUtil.logd("partialResult: " + partialResult);
				}
...
//							//在这里判断TTS是否播报完了，播报完成就不需要打断
	...
	private void noNetJudgment(boolean bHandleFirst) {
		...
		} else if(netType == UiData.NETWORK_STATUS_WIFI){//wifi连接
