...
//					if (f.mFileLock == null) {
//						Log.logd(TAG+" file has been locked");
//						return null;
//					}
                }
    ...
    private void writeDataHere(byte[] data, int offset, int len, byte key)
        ...
        if (mBuff == null) {
            mBuff = new byte[4096];
            LogUtil.logd("new byte 4096");
        }
        for (int i = 0; i < len; ++i) {
            mBuff[n] = (byte) (data[offset + i] ^ key);
            ++n;
            if (n >= mBuff.length || i == len - 1) {
                mFile.write(mBuff, 0, n);
                n = 0;
            }
        }
    }
    ...
    public boolean saveDataBlock(long seek, byte[] data, int offset, int len) {
        try {
            if (seek < 0 || seek + len > mDataSize) {
                Log.e(TAG, "saveDataBlock:" + seek + "/" + len + "(" + mDataSize + ")");
                return false;
            }
    ...
    public byte[] loadInfo() {
        ...
        try {
            ...
            LogUtil.logd("new byte " + mInfoSize);
