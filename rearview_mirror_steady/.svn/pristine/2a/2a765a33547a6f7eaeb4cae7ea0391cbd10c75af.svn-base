...
public class ProcessMemoryMonitor {
    ...
    private static final String TAG = "Music:ProcessMemoryMonitor ";
    ...
    private Map<String, Integer> mMapNamePid = new HashMap<>();
    private Map<Integer, String> mMapPidName = new HashMap<>();
    ...
    public void addMonitorProcess(MonitorProcess monitorProcess) {
        if (monitorProcess == null || TextUtils.isEmpty(monitorProcess.mProcessName)) {
            LogUtil.loge(TAG + "processName can't be null!");
            return;
        }
        if (!mIsInited) {
            init();
        ...
        public void run() {
            ...
            synchronized (mLock) {
                ...
                for (int i = 0; i < memoryInfoArray.length; i++) {
                    ...
                    LogUtil.logd(TAG + "process:" + pids[i] + processName +
