...
public class ReportManager {
    ...
    private static final String TAG = "music:ReportManager:";
    ...
    public void reportImmediate(Object event) {
        String json = JsonHelper.toJson(event);
        Logger.d(TAG, "reportImmediate:%s", json);
        ReportUtil.doReportImmediate(com.txz.report_manager.ReportManager.UAT_MUSIC, json.getBytes());
    }
    ...
    public void reportImmediate(String str) {
        Logger.d(TAG, "reportImmediate:%s", str);
        ReportUtil.doReportImmediate(com.txz.report_manager.ReportManager.UAT_MUSIC, str.getBytes());
    }
    ...
    public void reportAudioPlay(Audio currentAudio, int type) {
        if (currentAudio != null && currentAudio.getSid() != 0) {
            ...
            if (PlayInfoManager.getInstance().getCurrentAlbum() != null) {
                reportHistory.parent_album_id = PlayInfoManager.getInstance().getCurrentAlbum().getId();
                reportHistory.parent_album_sid = PlayInfoManager.getInstance().getCurrentAlbum().getSid();
            }
            reportHistory.audioId = currentAudio.getId();
            reportHistory.type = type;

//            reportRepeatedly(reportHistory);
            reportImmediate(reportHistory);
        }
    }
}
