...
public class SDKFloatViewInnerV2 extends LinearLayout implements ISDKFloatView{
	...
	private View mVoiceAssistant;
	ViewFactory.ViewAdapter adapter;
	...
	public SDKFloatViewInnerV2(final Context context) {
		super(context);
		mWinManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
		mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
		adapter = WinLayoutManager.getInstance().getFloatView().getView(new ViewData(ViewData.TYPE_FLOAT_VIEW));
		mVoiceAssistant = adapter.view;
		initView(context);
	}
	...
	public void setImageBitmap(final String normal, final String pressed) {
		if(adapter != null && adapter.object instanceof IFloatView){
			((IFloatView)adapter.object).setImageBitmap(normal, pressed);
		}
	...
	private void initView(Context context) {
		LayoutInflater.from(context).inflate(R.layout.win_sdk_float_view_v2, this);
		View root = findViewById(R.id.llSDK_Float_View_Root);
		flContent = (FrameLayout) root
				.findViewById(R.id.flContent);
		flContent.addView(mVoiceAssistant);
		mTestFlagView = root.findViewById(R.id.txtSDK_Float_View_TesfFlag);
		mVoiceAssistant.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
			@Override
			public void onGlobalLayout() {
				mVoiceAssistant.getViewTreeObserver().removeOnGlobalLayoutListener(this);
				mWidth = mVoiceAssistant.getWidth();
				mHeight = mVoiceAssistant.getHeight();
				LogUtil.loge("width:"+mWidth + "height:"+ mHeight);
			}
		});
		...
		getViewTreeObserver().addOnPreDrawListener(new OnPreDrawListener() {
			...
			public boolean onPreDraw() {
				...
				try {
					...
					mLp.x = cacheX;
					mLp.y = cacheY;
					mWinManager.updateViewLayout(SDKFloatViewInnerV2.this, mLp);
					historyX = mLp.x;
					historyY = mLp.y;
