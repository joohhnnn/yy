...
public class MyFavourActivity extends BaseBarActivity implements OnRefreshListener, OnLoadMoreListener, Observer {
    ...
    public RecyclerView.Adapter getAdapter() {
        itemAudioAdapter = new ItemAudioAdapter<FavourBean>(this, favourBeans) {
            ...
            public void bindView(AudioViewHolder v, FavourBean favourBean, int position) {
                ...
                v.mIvDelete.setImageDrawable(GlobalContext.get().getResources().getDrawable(R.drawable.ic_favorite_small));
                ...
                if (favourBean != null && favourBean.getAudio() != null) {
                    ...
                    if (currentAudio != null && favourBean.getAudio().equals(currentAudio)) {
                        ...
                        if (ScreenUtils.isPhonePortrait()) {
                            isPhoneSettingItemTitle(v, favourBean.getAudio(), true);
                        } else {
                        ...
                        if (ScreenUtils.isPhonePortrait()) {
                            isPhoneSettingItemTitle(v, favourBean.getAudio(), false);
                        } else {
        ...
        itemAudioAdapter.setOnDeleteListener(new ItemAudioAdapter.OnDeleteListener<FavourBean>() {
            @Override
            public void onDelete(FavourBean favourBean) {
                LogUtil.logd(getActivityTag() + " setOnDeleteListener " + favourBean);
                //从数据库中删除
                final Audio audio = favourBean.getAudio();
                FavorHelper.unfavor(audio, EnumState.Operation.manual);
                ReportEvent.clickMineFavourDelete(audio, audio.getName());
            }
        });
        itemAudioAdapter.setOnItemClickListener(new ItemAudioAdapter.OnItemClickListener() {
            ...
            public void onClick(int position) {
                ...
                AppLogic.runOnBackGround(() -> {
                    ...
                    ReportEvent.clickMineFavourAudio(audios.get(position), audios.get(position).getName());
                });
    ...
    public int getLayout() {
        if (ScreenUtils.isPhonePortrait()) {
            return R.layout.act_favour_phone_portrait;
        }
    ...
    private void initLinstener() {
        ...
        loadingView.setEmptyHintListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra(Utils.KEY_TYPE, HomeActivity.MUSIC_i);
    ...
    private void requestData() {
        loadingView.showLoading(R.drawable.fm_local_scan, R.drawable.fm_local_scan_logo, "加载收藏的音乐中...");
            ...
            public void onResponse(List<FavourBean> list) {
                if (CollectionUtils.isNotEmpty(list)) {
                    ...
                    for (int i = list.size() - 1; i >= 0; i--) {
                        //如果收藏的歌曲，不存在本地中，则不展示
                        FavourBean favourBean = list.get(i);
                        if (Utils.isLocalSong(favourBean.getSid())) {
                            if (favourBean.getAudio() == null || !FileUtils.isExist(favourBean.getAudio().getStrDownloadUrl())) {
                                //不存在 或者路径不存在则删除这首歌曲
                                list.remove(i);
                            }
                        }
    ...
    protected String getActivityTag() {
        return "MyFavourActivity#" + this.hashCode() + "/收藏的音乐";
    }
                    ...
                    if (Utils.isSong(obj.getSid())) {
                        ...
                        if (index >= 0) {
                            ...
                            if (favourBeans.size() == 0) {
                                loadingView.showEmpty("你还没有收藏的内容哦", R.drawable.fm_mine_favour_none, "去看看");
                                isPhoneSettingTitle(0);
                            } else {
                                itemAudioAdapter.notifyItemRangeChanged(index, favourBeans.size() - index);
                                isPhoneSettingTitle(favourBeans.size());
                            }

                            //坑人：产品，想看4.2.1的点击我的收藏页面点击歌曲，注意这里不切歌了。
...
//                            }
    ...
    public void isPhoneSettingTitle(int num) {
        if (tvTitle != null) {
            if (ScreenUtils.isPhonePortrait()) {
                tvTitle.setTextColor(Color.WHITE);
                if (num == 0) {
