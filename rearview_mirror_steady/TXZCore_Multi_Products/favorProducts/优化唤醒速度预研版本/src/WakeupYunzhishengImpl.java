...
import com.txznet.test.wakeup.su.TXZAudioSource;
...
public class WakeupYunzhishengImpl implements IWakeup{
	...
	public int initWakeup(IInitCallback oRun) {
		...
		mSpeechUnderstander.setOption(SpeechConstants.ASR_OPT_RECORDING_PAC_SIZE, 1200);
	...
	private long mSpeechDetectedTime = 0;
	private long mSpeechEndTime = 0;
	
	private void onSpeechBegin(){
		mSpeechDetectedTime = SystemClock.elapsedRealtime();
	...
	private void onSpeechEnd(){
		...
		if (mSpeechDetectedTime > 0 && mSpeechEndTime - mSpeechDetectedTime > 400){
			LogUtil.logd("fill silence data to engine");
			mConfig.setSilenceLength(6400 + 3200);
		}
