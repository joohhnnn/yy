...
public class WeatherRefresher2 {
	...
	public void updateData(String mContent, WeatherViewHolder v) {
		...
		try {
			/*if (mWeatherInfos.uint32FocusIndex == 0) {
                ...
                LogUtil.logd(WinLayout.logTag+ "showTemp");
				refreshTemp(
						v,
						mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].int32CurTemperature);
			} else {
				LogUtil.logd(WinLayout.logTag+ "dismissTemp"+mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].int32CurTemperature+"--"+mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].int32LowTemperature+"--"+mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].int32HighTemperature);
			...
			v.mAirQuality.setText(LanguageConvertor.toLocale("  "+(pm25==0? "" : pm25 + " ")+(TextUtils.isEmpty(mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].strAirQuality)?"未知":mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].strAirQuality)+"  "));

			v.mAirQuality.setBackground(getBgDrawable(TextUtils.isEmpty(mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].strAirQuality)?"未知":mWeatherInfos.rptMsgWeather[mWeatherInfos.uint32FocusIndex].strAirQuality));
	 ...
	 * @param currentIndex
	 */
	private void refreshThreeDay(WeatherViewHolder v, String date, Integer todayLow,
