...
public class FileMediaResponse extends MediaResponseBase {
	...
	protected long getLength(OutputStream out) throws IOException {
		...
		if (!f.exists()) {
			...
			mSess.player.notifyError(new MediaError(MediaError.ERR_FILE_NOT_EXIST, "file not exist", "文件不存在"));
		}
		if (path.toLowerCase(Locale.CHINESE).endsWith(".tmd")) {
			long ret = TmdFile.getDataSize(f);
			if (ret < 0) {
				mSess.player.notifyError(new MediaError(MediaError.ERR_CHECK_ERROR, "file check error", "文件校验错误"));
			}
			return ret;
		} else {
			return f.length();
