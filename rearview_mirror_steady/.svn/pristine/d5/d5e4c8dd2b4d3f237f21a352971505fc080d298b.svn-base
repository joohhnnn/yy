...
public class AsrWakeupEngine {
	...
	private Handler compileHandler = null;
	...
	private AsrWakeupEngine() {
		...
		compileThread.start();
		compileHandler = new Handler(compileThread.getLooper());
	...
	public synchronized int initialize(AsrAndWakeupIIintCallback oRun) {
		if (mEngine == null){
			if (!TextUtils.isEmpty(ProjectCfg.getIflyAppId()) && !TextUtils.isEmpty(ProjectCfg.getYunzhishengAppId())){
				if (ProjectCfg.getVoiceEngineType() == 0)
					strEngine = STR_TXZ_ENGINE;
				else if (ProjectCfg.getVoiceEngineType() == UiEquipment.VET_IFLY)
					strEngine = STR_TXZ_ENGINE;
				else if (ProjectCfg.getVoiceEngineType() == UiEquipment.VET_YUNZHISHENG)
					strEngine = STR_YZS_ENGINE;
				else
					strEngine = STR_TXZ_ENGINE;
			}else{
