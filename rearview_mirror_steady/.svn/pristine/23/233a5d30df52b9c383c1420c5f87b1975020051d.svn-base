    ...
    public void prepareAsync() {
        ...
        PlayItemManager.getInstance().createPlayItem(mAudio, new PlayItemManager.IPlayItemListener() {
            ...
            public void onError(Error error) {
                Logger.d("test:sessionId:error:", getSessionId() + "|---|" + error.toString());
    ...
    public synchronized void release() {
        ...
        if (mServiceMessenger != null) {
            LogUtil.logd("service_id=" + service_id + "/"
                    + Utils.getProcessIdByPkgName(Constant.PACKAGE_PLAYER));
            if (service_id == Utils.getProcessIdByPkgName(Constant.PACKAGE_PLAYER)) {
                try {
                    isBlock = false;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SERVICE_ACTION_PROGRESS:
                    ...
                    /*long position1 = msg.getData().getLong(KEY_POSTION, 0);
                    long duration = msg.getData().getLong(KEY_DURATION, 0);
                    int needPosition = TXZAudioPlayer.NEED_BUFFER_DATA_TIME / 1000 * Constant.TIME_UNIT;
                    if (isNeedNextAudioPreloadData && ((position1 + needPosition) >= duration)) {
                        LogUtil.logd(Constant.PRELOAD_TAG + "position:" + position1 + ",duration=" + duration + ",needposition:" + (position1 + needPosition));
                        Audio nextAudio = PlayInfoManager.getInstance().getNextAudio(false);
                        if (nextAudio != player.mAudio) {
                            isNeedNextAudioPreloadData = false;
//                            DataInterfaceBroadcastHelper.sendStartPreloadNextAudioInfo(nextAudio);
                            DownloadManager.getInstance().download(nextAudio, new DownloadManager.OnDownloadListener() {
                                @Override
                                public void onSuccess() {
                                    Logger.d(Constant.PRELOAD_TAG, "success");
                                }

                                @Override
                                public void onError(int code) {
                                    Logger.d(Constant.PRELOAD_TAG, "onError:" + code);
                                }
                            });
                        }
                    }*/
