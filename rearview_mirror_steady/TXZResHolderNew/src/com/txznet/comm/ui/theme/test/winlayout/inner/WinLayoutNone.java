...
public class WinLayoutNone extends IWinLayout {
	...
	private LinearLayout mRootLayoutWrapper;
...
//	private ImageView ivHelp;
	...
	// private boolean mustShowContent;
...
//			llParams.topMargin = (int) LayouUtil.getDimen("x10");
	...
	public void updateScreenType(int type) {
		...
		LogUtil.logd("updateScreenType= " + type);
...
//		int h = mRootLayout.getMeasuredHeight();
		...
		// 	h = -1;
		// }
		RecordWin2Manager.getInstance().updateDisplayArea(x , y , -1, -1);
		lastScreenType = type;
	}
...
//	String beforeFLoatType;
	...
	public Object addView(int targetView, View view,
		...
		LogUtil.logd("classname= " + view.getClass().getSimpleName());
		...
		// if (tag instanceof Integer && tag == ViewData.TYPE_CHAT_TO_SYS_PART) {
		...
		// if (mustShowContent && tag instanceof Integer && (int)tag == ViewData.TYPE_CHAT_TO_SYS_PART) {
	...
	public void setGuideText(){
		...
		if (WinLayout.getInstance().vTips != null) {
			...
			if (text.startsWith("请说")){
				...
				viewAdapter = ChatFromSysView.getInstance().getTipsView(tipViewData);
			}else {
