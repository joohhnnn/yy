...
public class SingleRadioFragment extends BaseDataFragment<Category> implements
		OnClickListener {
	...
	public static class MyInstance {
		public static SingleRadioFragment instance = new SingleRadioFragment();
	}
	...
	private void setNavViewList(View[] views){
		if(!isHidden()){
			LogUtil.logd("NAVBtn:radio fragment set views " + (views == null ? 0 : views.length));
	...
	public void initPopView() {
		...
		OnItemClickListener mOnGridItemClickListener = new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				List<Category> result = ((Category) filterGridViewAdapter
						.getItem(position)).getArrChild();
				if (CollectionUtils.isEmpty(result)) {
					mCategory = (Category) filterGridViewAdapter
							.getItem(position);
				} else {
					mCategory = result.get(0);
				}
				getAlbumTask = new MyAsyncTask<Void, List<Album>>() {
					@Override
					protected void onPreExecute() {
						filterGridViewAdapter.setSelectedIndex(position);
						setTypeName(((Category) filterGridViewAdapter
								.getItem(position)).getDesc());
...
//					adapter.notifyItemInserted(lastVisibleItem + 1);
	...
	public void initData() {
		...
		mCategorys.clear();
		setTypeName("");
		manager = new LinearLayoutManagerWrapper(getActivity(), "SingleRadioFragment");
		...
		if (CollectionUtils.isNotEmpty(result)) {
			...
			for (Category category : result) {
				if (category.getCategoryId() != 100000
						&& category.getCategoryId() != 1) {// 过滤掉音乐和本地
			...
			if (CollectionUtils.isNotEmpty(mCategorys)
				...
				if (CollectionUtils.isNotEmpty(mCategorys.get(lastPostion)
						.getArrChild())) {
					mCategory = mCategorys.get(lastPostion).getArrChild()
							.get(0);
				} else {
					mCategory = mCategorys.get(lastPostion);
				}
				setTypeName(mCategorys.get(lastPostion).getDesc());
			} else {
				LogUtil.loge("该类型没有子分类size:" + mCategorys.size()
						+ ",lastPostion:" + lastPostion);
				showNetTimeOutView("配置错误");// 展示网络请求失败的图示
				return;
			}
	...
	public void onHiddenChanged(boolean hidden) {
		LogUtil.logd(TAG + "radio onHiddenChanged:" + hidden);
