        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (viewConfiguration == null) {
                    viewConfiguration = ViewConfiguration.get(GlobalContext.get());
                }
                if (mOnClickListener != null && SystemClock.currentThreadTimeMillis() - down < ViewConfiguration.getLongPressTimeout()) {
                    if (Math.abs(event.getX() - offsetX) < viewConfiguration.getScaledTouchSlop() && Math.abs(event.getY() - offsetY) < viewConfiguration.getScaledTouchSlop()) {
