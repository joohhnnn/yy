...
import com.txznet.music.adpter.HistoryAdapter;
...
public class RadioHistoryFragment extends BaseDataFragment<Audio> {
	// 使用单例
	public static class MyInstance {
		public static RadioHistoryFragment instance = new RadioHistoryFragment();
	}
	...
	public List<Audio> getDataFromLocal() {
		List<Integer> sidType = Utils.getSidType(PlayConf.NEWS_TYPE);
		sidType.addAll(Utils.getSidType(PlayConf.RADIO_TYPE));
		...
		LogUtil.logd(TAG+"find history radio :"+sb.toString());
	...
	public void initData() {
		...
		mLv_list.setLayoutManager(new LinearLayoutManagerWrapper(getActivity(), "RadioHistoryFragment"));
	...
	private void setNavViewList(){
		if(!isHidden()){
			...
			LogUtil.logd("NAVBtn:radio history fragment set view list");
	...
	public void onPause() {
		...
		LogUtil.logd("NAVBtn:radio history fragment delete view list");
				...
				if (current != null && mAudios != null&&!Utils.isSong(current.getSid())) {
					if (current != null) {
			            for (int i = 0; i < mAudios.size(); i++) {
			                if (current.getAlbumId().equals(mAudios.get(i).getAlbumId())) {
			                	mAudios.remove(i);
			                	break;
			                }
			            }
