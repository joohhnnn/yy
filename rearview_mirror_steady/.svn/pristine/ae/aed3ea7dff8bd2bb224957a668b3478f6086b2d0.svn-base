...
public class BDLocationUtil {
	...
	public static LocationInfo Convert_BDLocation_To_LocationInfo(
		...
		LocationInfo locationInfo = new LocationInfo();
		{
			try {
				SimpleDateFormat sdf = new SimpleDateFormat(
						"yyyy-MM-dd HH:mm:ss");
				Date d = sdf.parse(location.getTime());
				locationInfo.uint32Time = (int) (d.getTime() / 1000);
			} catch (Exception e) {
				// locationInfo.uint32Time = (int) (System.currentTimeMillis() / 1000);
			}
		}
		...
		locationInfo.msgGpsInfo.fltSpeed = location.getSpeed() / 3.6F;
