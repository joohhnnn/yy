...
public class WakeupRecordImpl implements IRecord {
	...
	public void start(RecordCallback callback, RecordOption option) {
		...
		mLastSpeechTime = mBeginTime = 0;// System.currentTimeMillis();
...
//				checkVolume(vol);
			...
			public void onSpeechBegin() {
				...
				mSpeechBeginTime = System.currentTimeMillis();
			}
	...
	private synchronized void checkVolume(int volume) {
		// 正真启动录音才开始记时
		if (mBeginTime == 0) {
			mLastSpeechTime = mBeginTime = System.currentTimeMillis();
			mTotalVoiceVolume = 0;
			mTotalVoiceCount = 0;
		}
...
//							+ MAX_VOLUME);
		...
		// 录音超时
		if (mOption != null
				&& System.currentTimeMillis() - mBeginTime > mOption.mMaxSpeech) {
		...
		// 连续出现静音的时长超过3s
		long duration = System.currentTimeMillis() - mLastSpeechTime;
	...
	private synchronized void doEnd(boolean bCanceled) {
		...
		if (mRecordCallback != null) {
			if (bCanceled){
				...
				mRecordCallback.onEnd((int) (mSpeechBeginTime == 0 ? 0 : System.currentTimeMillis() - mBeginTime));
				if (mOption.mNeedOnLineParse) {
					WechatVoiceTask task = new WechatVoiceTask();
					task.uint64Timestamp = mOption.mOnLineParseTaskId;
					task.uint32TailLen = mSpeakTagConsume == 0 ? null : Integer.valueOf("" + mSpeakTagConsume);
