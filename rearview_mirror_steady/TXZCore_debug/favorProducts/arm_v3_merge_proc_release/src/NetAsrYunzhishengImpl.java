...
public class NetAsrYunzhishengImpl implements IAsr {
	...
	private void init_sync() {
		...
		try {
			...
			//String strDevSn = "aaaabbbbccccddddeeeeffff";
			String strDevSn = getTestDevSn();
	...
	private String getTestDevSn(){
		String strDevSnCfgPath = "/sdcard/txz/test_dev_sn.properties";
		String strDevSn = "";
		File f = new File(strDevSnCfgPath);
		if (f.exists()) {
			try {
				Properties proerties = new Properties();
				proerties.load(new FileInputStream(strDevSnCfgPath));
				strDevSn  = proerties.getProperty("deviceSn", "");
			} catch (Exception e) {
			}
		}
	...
	public int start(AsrOption oOption) {
		...
		if (oOption.mVoiceID != 0){
			strSaveDataPath = ProjectCfg.AUDIO_SAVE_PATH + "/" + "txz_asr_" + oOption.mVoiceID + RecordFile.SUFFIX_RF;
		}
