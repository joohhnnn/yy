...
import com.txznet.txz.component.asr.mix.local.LocalAsrUVoiceImpl;
import com.txznet.txz.component.asr.mix.net.NetAsrBaiduImpl;
import com.txznet.txz.component.asr.mix.net.NetAsrTencentImpl;
...
public class TextResultHandle extends IModule {
	...
	// 识别电话的类型
	public static final int CALL_LOCAL_YZS_CH = 1;//离线中文
	public static final int CALL_LOCAL_YZS_EN = 2;//离线英文
	public static final int CALL_NET_YZS = 3;//在线云知声
	public static final int CALL_NET_IFLY = 4;//在线讯飞
	...
	public int initialize_BeforeStartJni() {
		...
		regEvent(UiEvent.EVENT_ACTION_EQUIPMENT,
				UiEquipment.SUBEVENT_RESP_FLOW_INFO);
	...
	public int onEvent(int eventId, int subEventId, byte[] data) {
		...
		case UiEquipment.SUBEVENT_NOTIFY_SERVER_CONFIG_UPDATE:
			try {
				...
				if (pbServerConfig.uint32TxzNlpLevel != null){
					...
					mTxzNlpLevel = IText.PRIORITY_LEVEL_NORMAL;
				}
		...
		case UiEquipment.SUBEVENT_RESP_WEATHER:
			try {
				...
				if (WeatherManager.getInstance().handleWeatherData(weatherData)) {
				}else {
					NetDataManager.getInstance().handleWeatherData(weatherData);
				}
			} catch (InvalidProtocolBufferNanoException e) {
				WeatherManager.getInstance().handleWeatherData((UiEquipment.Resp_Weather)null);
				JNIHelper.logd("server weather parse error:"+e.getMessage());
			}
			break;
		case UiEquipment.SUBEVENT_RESP_FLOW_INFO:
			try {
				EquipmentManager.Resp_SIMQueryFlowInfo flowInfo = EquipmentManager.Resp_SIMQueryFlowInfo.parseFrom(data);
				NetDataManager.getInstance().handleFlowInfoData(flowInfo);
			} catch (InvalidProtocolBufferNanoException e) {
				LogUtil.logw("server flowInfo parse error:"+e.getMessage());
			}
			break;
		default:
			break;
	...
	public void parseText(String text, ITextCallBack callBack) {
		...
		fixStrVoiceData(parseData, text);
		task.data = parseData;
	...
	public void parseText(VoiceParseData parseData, int moduleMask, ITextCallBack callBack){
		if (!mInited) {
			return;
		}
		ParserTask task = new ParserTask();
		...
		parseData.floatTextScore = TEXT_SCORE_INVALID;
		parseData.uint32Sence = VoiceData.GRAMMAR_SENCE_DEFAULT;
		fixStrVoiceData(parseData, parseData.strText);
		task.data = parseData;
		task.callBack = callBack;
		task.preCallBack = sysTextCallBack;
		task.moduleMask = moduleMask;
	...
	private void fixStrVoiceData(VoiceParseData parseData, String text) {
		...
		parseData.strVoiceData = json.toString();
	}
	...
	private boolean cutOneShotKw(VoiceParseData oVoiceData){	
		...
		if (TextUtils.isEmpty(oVoiceData.strText)) {
			JNIHelper.loge("instantAsr::curOneShotKw: strText is empty");
			return false;
		}
		...
		if (WakeupKwType.KW_TYPE_ONESHOT_ONLY.ordinal() == directAsrType) {
			if (TextUtils.isEmpty(strOneShotKw)) {
				return false;
			}

			int oneShotKwLength = strOneShotKw.length();

			if (strText.length() <= oneShotKwLength) {
			...
			if (comparePinYin(strOneShotKw,
					strText.substring(0, oneShotKwLength))) {
				oVoiceData.strText = oVoiceData.strText.substring(oneShotKwLength);
		...
		if (WakeupKwType.KW_TYPE_DIRECTASR_FRONT.ordinal() == directAsrType
			...
			if (TextUtils.isEmpty(strDirectAsrKw)) {
				return false;
			}
	...
	public void handleResult(VoiceParseData parseData, ITextCallBack callBack) {
		...
		// WTF,这个异常不应该存在的
		if (mTxzScene == null) {
			MonitorUtil.monitorCumulant("text.handle.E.handleEmpty");
			handleError("empty");
			return;
		}
		...
		if (SimManager.getInstance().mAsrPercent != -1) {
			if (Math.random() * 100 > SimManager.getInstance().mAsrPercent) {
				...
				MonitorUtil.monitorCumulant("text.handle.I.asrPercent");
				handleError("unknown");
				return;
			}
		}
		...
		if (mCurrentMask == 0) {
			MonitorUtil.monitorCumulant("text.handle.I.nomask");
			cancelHandleResult();
			return ;
		}
	...
	private int getMaskAndSetTextByScene() {
		...
		if((mask & (MODULE_LOCAL_HIGH_MASK | MODULE_LOCAL_NORMAL_MASK)) != 0 ){
			//设置本次识别的文本给native层，方便之后native层语义的处理
			UiData.Test_Keyword testKeyword = new UiData.Test_Keyword();
			testKeyword.strText = mTxzScene.strText;
			NativeData.getNativeData(UiData.DATA_ID_VOICE_SET_RAW_TEXT,testKeyword);
		}
	...
	public VoiceParseData voiceDataToTXZVoiceData(VoiceParseData parseData) {
		...
		case VoiceData.VOICE_DATA_TYPE_UVOICE_LOCAL_JSON:
			return LocalAsrUVoiceImpl.uvoiceDataToTxzScene(parseData);
		case VoiceData.VOICE_DATA_TYPE_BAIDU_SCENE_JSON:
			return NetAsrBaiduImpl.baiduDataToTxzScene(parseData);
		case VoiceData.VOICE_DATA_TYPE_TENCENT_SCENE_JSON:
			return NetAsrTencentImpl.tencentDataToTxzScene(parseData);
		default:
			try {
	...
	private int getMaskByType(int dataType) {
		...
		case VoiceData.VOICE_DATA_TYPE_UVOICE_LOCAL_JSON:
			isLocal = true;
			if (mTxzScene.uint32TextMask != null && mTxzScene.uint32TextMask != 0) {
				mask = mTxzScene.uint32TextMask;
			}else {
...
//			if (object.has("answer"))
		...
		} else if (scene.equals("train")) {
			disable = checkDisableScene(UiEquipment.CONTROL_TRAIN);
		} else if (scene.equals("flight")) {
			disable = checkDisableScene(UiEquipment.CONTROL_PLANE);
		} else if (scene.equals("news")) {
			disable = checkDisableScene(UiEquipment.CONTROL_NEWS);
		} else if (scene.equals("system")) {
			disable = checkDisableScene(UiEquipment.CONTROL_SYS_CTL);
		} else if (scene.equals("airC")) {
			disable = checkDisableScene(UiEquipment.CONTROL_AIRC);
		} else if (scene.equals("reminder")) {
			disable = checkDisableScene(UiEquipment.CONTROL_REMINDER);
		} else if (scene.equals("raw")) {//主动请求的语义场景，非正常的语义场景
			disable = false;
		}
		...
		if (!checkDisableScene(UiEquipment.CONTROL_CHAT)) {
			if (object.has("style")) {
				...
				try {
					...
					}else if (mStyle.equals("poem")){//诗词
						ret = checkDisableChatScene(UiEquipment.CHAT_TYPE_POEM);
					}
				} catch (JSONException e) {
					e.printStackTrace();
				}
	...
	private void compareJson(VoiceParseData parseData, int priority) {
		...
		if (parseData.floatTextScore > mTxzScene.floatTextScore) {
			...
			} else if (parseData.uint32DataType == VoiceData.VOICE_DATA_TYPE_TXZ_SENCE_NEW) {
				//当后台分数较低时，需要将localCmd字段赋值到分数高的语义结果中，目前是四维车控家在用
			    if (!TextUtils.isEmpty(parseData.strVoiceData)) {
                    JSONBuilder jsonBuilder = new JSONBuilder(parseData.strVoiceData);
                    String localCmd = jsonBuilder.getVal("localCmd", String.class);
                    if (!TextUtils.isEmpty(localCmd)) {
                        if (!TextUtils.isEmpty(mTxzScene.strVoiceData)) {
                            mTxzScene.strVoiceData = new JSONBuilder(mTxzScene.strVoiceData).put("localCmd", localCmd).toString();
                        }
                    }
                }
            }
	...
	private void onRespResult() {
		...
		ReportUtil.doVoiceReport(new ReportUtil.Report.Builder()
				.setKeywords(TextTxzHighLocalImpl.getRealText(mStrText))
				.setAction("exec").putExtra("scene", "command").setSessionId()
				.putExtra("_rt", "voice").buildVoiceReport(), 
				mCurrTask.data.uint32DataType, mCurrTask.data.uint64VoiceFileId);
...
//			return 0;
	...
	//		 VoiceData.SUBEVENT_VOICE_PARSE, txzScene);
			...
			if (VoiceData.VOICE_DATA_TYPE_TXZ_SENCE_NEW == txzScene.uint32DataType) {
				...
				jsonReport.put("recordType", UiRecord.RECORD_TYPE_ASR);
				jsonReport.put("sessionId", ReportUtil.mSessionId);
			}
			if(txzScene.uint64VoiceFileId != null){
				ReportUtil.doVoiceReport(txzScene.uint32DataType, txzScene.uint64VoiceFileId, jsonReport.toString().getBytes());
			}
