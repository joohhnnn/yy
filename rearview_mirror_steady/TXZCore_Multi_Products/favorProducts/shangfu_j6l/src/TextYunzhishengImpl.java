...
public class TextYunzhishengImpl implements IText {
	...
	public static VoiceParseData yzsDataToTxzScene(VoiceParseData parseData) {
		...
		if (rc != 0 && rc != 1)
			MonitorUtil.monitorCumulant(NetAsrYunzhishengImpl.MONITOR_ERROR+NetAsrYunzhishengImpl.MONITOR_NO_NLP + rc);
		switch (rc) {
		//操作成功
		case 0:
			break;
...
//							}
	...
	private static boolean parseOLCommonFunction(VoiceParseData voiceData,
        ...
        if(parseOLVideo(voiceData, rawJson)) {
        	return true;
		}
	...
	private static boolean parseOLVideo(final VoiceParseData voiceData, final JSONObject rawJson) {
		String strService = rawJson.getString("service");
		if (TextUtils.equals("cn.yunzhisheng.video", strService)) {
			String strVoiceText = rawJson.getString("text");
			JSONObject jsonIntent = rawJson.getJSONObject("semantic").getJSONObject("intent");
			if (jsonIntent != null) {
				String action = rawJson.getString("code").toLowerCase();
				JSONObject jsonResult = genVoiceJSONObject("video_ctrl", action,
						strVoiceText);
				JSONObject data = new JSONObject();
				data.put("name", jsonIntent.getString("name"));
				data.put("keyword", jsonIntent.getString("keyword"));
