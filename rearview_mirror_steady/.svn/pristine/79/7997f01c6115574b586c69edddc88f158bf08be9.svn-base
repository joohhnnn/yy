...
//	private Map<String, UdpAddress> mMapAddress = new HashMap<String, UdpConfiger.UdpAddress>();
	...
	private void initClient() {
		...
		ServiceManager.getInstance().addConnectionListener(new ServiceManager.ConnectionListener() {
			@Override
			public void onConnected(String serviceName) {
				LogUtil.logd(TAG + "onConnected :" + serviceName);
				if (ServiceManager.TXZ.equals(serviceName)) {
					mHandlerCheck.post(mTaskCheckConnection);
					JSONBuilder jsonBuilder = new JSONBuilder();
					jsonBuilder.put("processName", AppLogicBase.getProcessName());
					ServiceManager.getInstance().sendInvoke(ServiceManager.TXZ, "txz.udp.init", jsonBuilder.toBytes(),
							null);
				}
			}
			...
			public void onDisconnected(String serviceName) {
				LogUtil.logd(TAG + "onDisconnected :" + serviceName);
				if (ServiceManager.TXZ.equals(serviceName)) {
...
//	}
	...
	public int sendInvoke(int invokeMethod, byte[] data) {
		return sendInvoke(ServiceManager.TXZ,invokeMethod,data);
	}
	...
	public ServiceData sendInvokeSync(int invokeMethod, byte[] data) throws Exception{
		return sendInvokeSync(ServiceManager.TXZ, invokeMethod, data);
	}
	...
	public byte[] onInvoke(String packageName, String command, byte[] data) {
		if("comm.udp.initInfo".equals(command)){
			...
			LogUtil.logd(TAG + "processName:" + AppLogicBase.getProcessName() + ",udpId:" + mUdpId);
			return null;
		}
		return null;
	}
	...
	private boolean checkConnection() {
		...
		try {
			resp = mClient.sendInvoke(new UdpData(mUdpId,UdpData.INVOKE_SYNC, UdpData.CMD_CHECK_CONNECTION, 
					AppLogicBase.getProcessName().getBytes()),mServerAddr);
		} catch (Exception e) {
			e.printStackTrace();
			mIsInConnection = false;
			return false;
