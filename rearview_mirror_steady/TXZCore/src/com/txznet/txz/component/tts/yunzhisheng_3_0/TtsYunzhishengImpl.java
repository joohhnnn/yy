...
public class TtsYunzhishengImpl implements ITts {
	...
	private static final int BACK_SILENCE_TIME = TXZFileConfigUtil.getIntSingleConfig(TXZFileConfigUtil.KEY_TTS_BACK_BUFFER_TIME, 100); // 最大值为1000ms, 默认100ms
	private static final int FRONT_SILENCE_TIME = TXZFileConfigUtil.getIntSingleConfig(TXZFileConfigUtil.KEY_TTS_FRONT_BUFFER_TIME, 0); // 默认0ms
	...
	public int initialize(IInitCallback oRun) {
		...
		do{
			...
			String strDictPath = AppLogic.getApp().getApplicationInfo().dataDir + "/data/" + TTS_DICT_NAME;
		...
		mTTSPlayer.setTTSListener(new SpeechSynthesizerListener() {
				...
				case SpeechConstants.TTS_EVENT_PLAYING_END:
					...
					if (mTtsCallback != null){
						 Runnable oRun = new Runnable() {
								@Override
								public void run() {
									final ITtsCallback cb = mTtsCallback;
									if (cb != null){
										cb.onSuccess();
									}
									mIsBusy = false;
			...
			public void onError(int type, String errorMSG) {
				...
				case ErrorCode.TTS_ERROR_OFFLINE_CHANGE_SPEAKER_FAIL:
					...
					setTtsModel(null);
					return;
				case ErrorCode.TTS_ERROR_AUDIOSOURCE_OPEN://{"errorCode":-91101,"errorMsg":"播放线程打开audioSource出错"}
					mOutFatalTimes++;
					JNIHelper.loge("open audiosource occur serious fatal mOutFatalTimes : " + mOutFatalTimes);
	...
	public int start(int iStream, String sText, ITtsCallback oRun) {
        ...
        if (mOption != null && mOption.mPlayStartBufferTime != null){
        	mTTSPlayer.setOption(SpeechConstants.TTS_KEY_PLAY_START_BUFFER_TIME, mOption.mPlayStartBufferTime);
        }
        JNIHelper.logd("BUFFER_TIME : " + mTTSPlayer.getOption(SpeechConstants.TTS_KEY_PLAY_START_BUFFER_TIME));
	...
	private ITts.TTSOption mOption = new TTSOption();
	@Override
	...
	private   String ttsTextReplaceTel(String text){
		if(text.indexOf("电话") > -1 || text.indexOf("来电")> -1){
			...
			if(temp[0] >= 0 && (temp[1]-temp[0]) >= 8) {
				text = text.replace(text.substring(temp[0], temp[1]), "<tel>"+text.substring(temp[0], temp[1])+"</tel>");
			}
		}
		return text;
	}
