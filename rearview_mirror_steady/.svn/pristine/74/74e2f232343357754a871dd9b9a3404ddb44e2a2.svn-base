...
public class YZSAsrImpl extends SuperEngineBase {
		...
		AppLogic.runOnBackGround(new Runnable() {
			...
			public void run() {
				...
				do {
					...
					if (TextUtils.equals(LOCAL_ACTIVATE_DEVSN, strDevSn)) {// 离线激活v3引擎默认填充的唯一码:000000
	...
	public int initialize(IInitCallback oRun) {
		...
		mSpeechUnderstander.setOption(SpeechConstants.ASR_OPT_RECOGNIZE_MODEL_ID, 54);//加载54
...
//				.getApplicationInfo().dataDir + "/data/txz.dat");// 加载编译语法
	...
	private void setParam() {
        ...
        mSpeechUnderstander.setOption(SpeechConstants.ASR_OPT_FILTER_NAME, "search");
	...
	// 判断离线识别是否有说话
	private boolean isSilenceOnLocalAsr(Result result) {
		boolean bRet = false;
		do {
			if (result == null){
				break;
			...
			//不是离线识别结果
			if (result.mDataType != VoiceData.VOICE_DATA_TYPE_YUNZHISHENG_LOCAL_JSON) {
				break;
			}
			//是默认值
			if (result.mScore > -100 - DOUBLE && result.mScore < -100 + DOUBLE) {
				break;
			}
			//识别分数大于20.0f
			if (result.mScore > -15.0f) {
				break;
			}
	...
	private void onResult(Result result){
		// 没有说话判断
		if ((result.mScore > -20 - DOUBLE && result.mScore < -20 + DOUBLE ) || isSilenceOnLocalAsr(result)) {
			JNIHelper.logd("isSilenceOnLocalAsr");
