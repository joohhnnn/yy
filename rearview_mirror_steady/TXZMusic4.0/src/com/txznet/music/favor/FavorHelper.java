...
public class FavorHelper {
    ...
    //新增功能：
    // 【【同听4.4.0】【收藏】播放收藏的歌曲时取消收藏后，新增本地其他歌曲收藏，此时播放列表未展示新收藏的歌曲】
    //https://www.tapd.cn/21711881/bugtrace/bugs/view?bug_id=1121711881001003871
...
//                TtsUtil.speakResource("RS_VOICE_MUSIC_FAVOUR_TTS", Constant.RS_VOICE_MUSIC_FAVOUR_TTS, null);
                TtsUtil.speakTextOnRecordWin(Constant.RS_VOICE_MUSIC_FAVOUR_TTS, true, null);
            }
            //收藏成功的情况下，如果此时的播单来源为收藏列表，则顶部新增数据
            ...
            if (PlayInfoManager.getInstance().getCurrentScene() == PlayInfoManager.DATA_FAVOUR) {
                ...
                PlayInfoManager.getInstance().addAudio(audio, 0);//插在顶部
            }
            ...
            DBManager.getInstance().updateAudioFavour(audio);
            if (EnumState.Operation.sound == operation) {
                TtsUtil.speakTextOnRecordWin(Constant.RS_VOICE_MUSIC_UNFAVOUR_TTS, true, null);
//                TtsUtil.speakResource("RS_VOICE_MUSIC_UNFAVOUR_TTS", Constant.RS_VOICE_MUSIC_UNFAVOUR_TTS, null);
            }
        }
        PlayInfoManager.getInstance().setCurrentFavourMode(null, audio);
    }
    ...
    private static void favourFailure(boolean isFavour, EnumState.Operation operation) {
        if (isFavour) {
            ...
            if (EnumState.Operation.sound == operation) {
                TtsUtil.speakResource("RS_VOICE_MUSIC_FAVOUR_ERROR_TTS", Constant.RS_VOICE_MUSIC_FAVOUR_ERROR_TTS, null);
            }
        } else {
            ToastUtils.showShortOnUI(Constant.RS_VOICE_MUSIC_UNFAVOUR_ERROR_TTS);
            if (EnumState.Operation.sound == operation) {
                TtsUtil.speakResource("RS_VOICE_MUSIC_UNFAVOUR_ERROR_TTS", Constant.RS_VOICE_MUSIC_UNFAVOUR_ERROR_TTS, null);
            }
        }
    }
    ...
    private static void subscribeSuccess(Album album, boolean isSubscribe, EnumState.Operation operation) {
        ...
        if (isSubscribe) {
            if (EnumState.Operation.sound == operation) {
//                TtsUtil.speakResource("RS_VOICE_MUSIC_SUB_TTS", Constant.RS_VOICE_MUSIC_SUB_TTS, null);
                TtsUtil.speakTextOnRecordWin(Constant.RS_VOICE_MUSIC_SUB_TTS, true, null);
            }
            ...
            if (EnumState.Operation.sound == operation) {
//                TtsUtil.speakResource("RS_VOICE_MUSIC_UNSUB_TIPS", Constant.RS_VOICE_MUSIC_UNSUB_TIPS, null);
                TtsUtil.speakTextOnRecordWin(Constant.RS_VOICE_MUSIC_UNSUB_TIPS, true, null);
            }
            album.setOperTime(subscribeBean.getTimestamp());
        ...
        PlayInfoManager.getInstance().setCurrentFavourMode(album, null);
    ...
    private static void subscribeFailure(boolean isSubscribe, EnumState.Operation operation) {
        if (isSubscribe) {
            if (EnumState.Operation.sound == operation) {
                TtsUtil.speakResource("RS_VOICE_MUSIC_SUB_ERROR_TIPS", Constant.RS_VOICE_MUSIC_SUB_ERROR_TIPS, null);
            }

            ToastUtils.showShortOnUI(Constant.RS_VOICE_MUSIC_SUB_ERROR_TIPS);
        } else {
            if (EnumState.Operation.sound == operation) {
                TtsUtil.speakResource("RS_VOICE_MUSIC_UNSUB_ERROR_TIPS", Constant.RS_VOICE_MUSIC_UNSUB_ERROR_TIPS, null);
            }
    ...
    private static void reqData(final List<Album> albums, final boolean isSubscribe, final EnumState.Operation operation) {
        ...
        final List<BeSendBean> beSendBeans = new ArrayList<>();
            ...
            public void onResponse(ReqBesendBean data) {
                if (data != null) {
                    if (CollectionUtils.isNotEmpty(data.getArr_store_oper())) {
                        for (int i = 0; i < data.getArr_store_oper().size(); i++) {
                            BeSendBean beSendBean = data.getArr_store_oper().get(i);
                            if (beSendBeans.contains(beSendBean)) {
                                subscribeSuccess(beSendBeans.get(beSendBeans.indexOf(beSendBean)).getAlbum(), isSubscribe, operation);
                            }
                        }
                    }
                }
    ...
    public static void getFavourData(final int sid, final long id, final long reqTime, final int count, final SendFavourListener sendFavourListener) {
        if (isFirstGet(sid, id)) {//表明第一次
            sendToNetAll(new SendFavourListener() {
                ...
                @Override
                public void onError() {
                    //BUG:此处获取 的本地收藏有问题，导致重复刷，无网，直接就显示无网吧，未经产品同意。
                    sendFavourListener.onError();
...
//                                saveAudioTemps.add(responseFavourBeans.get(i).getAudio());
    ...
    public static void getSubscribeData(final int sid, final long id, final long operTime, final int count, final SendFavourListener sendFavourListener) {
        if (isFirstGet(sid, id)) {//表明第一次
            sendToNetAll(new SendFavourListener() {
                @Override
                public void onResponse(List list) {
                    getSubscribeDataFromNet(sid, id, operTime, count, sendFavourListener);
                }
    ...
    private static void getSubscribeDataFromNet(final int sid, final long id, long operTime, int count, final SendFavourListener sendFavourListener) {
        Assert.assertNotNull(sendFavourListener);
        ...
        reqFavour.setOperTime(operTime);
        if (NetworkUtil.isNetworkAvailable(GlobalContext.get())) {
            //有网则请求
            NetManager.getInstance().sendRequestToCore(Constant.GET_FAVOUR_LIST, reqFavour, new RequestCallBack<RespFavour>(RespFavour.class) {
     ...
     * 收藏，订阅
     */
    public static void favourAudio(Audio audio, Album album) {
        if (null != audio && Utils.isSong(audio.getSid())) {
            if (isSupportFavour(audio)) {
                //您应该说收藏哦
                FavorHelper.favor(audio, EnumState.Operation.sound);
            } else {
                TtsUtil.speakTextOnRecordWin("RS_VOICE_MUSIC_UNSUPPORT_FAVOUR_TIPS", Constant.RS_VOICE_MUSIC_UNSUPPORT_FAVOUR_TIPS, true, null);
            }
        } else if (null != album) {
            if (isSupportSubscribe(album)) {
                //即将为您订阅
                FavorHelper.subscribeRadio(album, EnumState.Operation.sound);
            } else {
                TtsUtil.speakTextOnRecordWin("RS_VOICE_MUSIC_UNSUPPORT_SUBSCRIBE_TIPS", Constant.RS_VOICE_MUSIC_UNSUPPORT_SUBSCRIBE_TIPS, true, null);
            }
        } else {
            TtsUtil.speakTextOnRecordWin("当前没有播放节目哦", true, null);
        }
    }
     ...
     * 取消收藏，取消订阅
     */
    public static void unFavourAudio(Audio audio, Album album) {
        if (null != audio && Utils.isSong(audio.getSid())) {
            if (isSupportFavour(audio)) {
                //您应该说收藏哦
                FavorHelper.unfavor(audio, EnumState.Operation.sound);
            } else {
                TtsUtil.speakTextOnRecordWin("RS_VOICE_MUSIC_UNSUPPORT_FAVOUR_TIPS", Constant.RS_VOICE_MUSIC_UNSUPPORT_FAVOUR_TIPS, true, null);
            }
        } else if (null != album) {
            if (isSupportSubscribe(album)) {
                //即将为您取消订阅
                FavorHelper.unSubscribeRadio(album, EnumState.Operation.sound);
            } else {
                TtsUtil.speakTextOnRecordWin("RS_VOICE_MUSIC_UNSUPPORT_SUBSCRIBE_TIPS", Constant.RS_VOICE_MUSIC_UNSUPPORT_SUBSCRIBE_TIPS, true, null);
            }
        } else {
