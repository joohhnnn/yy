...
public class HistoryAdapter extends MyBaseAdpater<Audio> {

	private RotateAnimation animation;
	private RotateAnimation backAnimation;

	public HistoryAdapter(List<Audio> data, Context ctx) {
		super(data, ctx);
	...
	public View getView(final int position, View convertView,
		...
		holder.mArtist.setText(song.getName());
		holder.mTitle.setText(StringUtils.isEmpty(song.getAlbumName())?"同行者推荐":song.getAlbumName());
...
//				HistoryAudioDBHelper.getInstance().remove(song.getId(),song.getSid());
				...
				HistoryAdapter.this.notifyDataSetChanged();// 更新组件
		...
		holder.mIv_show_delete.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (holder.mLl_delete.getVisibility() == View.VISIBLE) {
					...
					HistoryAdapter.this.notifyDataSetInvalidated();// 更新组件
				}
			}
		});
