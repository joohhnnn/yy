...
public class WakeupYunzhishengImpl implements IWakeup{
	private String TAG = "WakeupYunzhishengImpl";
	...
	private AIWakeupEngine mSpeechUnderstander = null;
	...
	*/
	...
	private AIWakeupListener mSpeechUnderstanderListener = new AIWakeupListener() {

        @Override
        public void onError(AIError error) {
            LogUtil.d(TAG,"error message:"+error);
        }
        ...
        public void onInit(final int status) {
        	if (status == AIConstant.OPT_SUCCESS){
        		...
        		WakeupYunzhishengImpl.this.onInit(false);
        	}
        }
        ...
        public void onWakeup(String recordId, final double confidence, final String wakeupWord) {
        	LogUtil.d(TAG,"wakeup foreground");
        	LogUtil.d(TAG,"recordId:"+recordId+";confidence:"+confidence+";wakeupWord:"+wakeupWord);
        	mWakeupText = "你好小新";
        	mWakeupTime = 500;
        	mWakeupScore = -2.9f;
        ...
        public void onBufferReceived(byte[] buffer) {
        	//LogUtil.d(TAG, "onBufferReceived: "+buffer.length);
        	onVolume(10);
        ...
        public void onWakeupEngineStopped() {
        	mSpeechUnderstander.start();//在这里启动下一轮唤醒
        }
	...
	public int initWakeup(IInitCallback oRun) {
        ...
        DUILiteSDK.init(AppLogic.getApp(), "d843ce414af0275398baf12f5a545967","278570221",new DUILiteSDK.InitListener() {
            @Override
            public void success() {
            	LogUtil.d(TAG,"授权成功");
            }
		...
		if (!AIWakeupEngine.checkLibValid()) {
			LogUtil.e(TAG,"so加载失败");
        } else {
        	mSpeechUnderstander = AIWakeupEngine.createInstance();
        	mSpeechUnderstander.setWakeupWord(new String[]{"ni hao xiao xin"}, new String[]{"0.1"});
            //mEngine.setResBinPath("/sdcard/aispeech/wakeup1.bin");//设置唤醒资源的绝对路径,包含文件名。默认在assets目录下，无需配置
        	mSpeechUnderstander.init(mSpeechUnderstanderListener);
        }
...
//				Environment.getExternalStorageDirectory().getPath()+"/referW.pcm");
