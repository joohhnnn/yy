...
public class WakeupManager extends IModule {
	...
	private String mInterruptKw = null;
...
//		updateProtectKeywords(kws);
	...
	private boolean startInner() {
		...
		if (isInitSuccessed() == false
			...
			String busyReason = String
					.format("wakeup is busy, waiting for idle: initSuccessd=%b; enable=%b; AsrBusy=%b; tts=%b; record=%b; call=%b, released=%b",
	...
	private boolean isMainWakeupKw(String text){
		try {
			if (mWakeupKeywords_Sdk != null && mWakeupKeywords_Sdk.length > 0) {
				for (String kw : mWakeupKeywords_Sdk) {
					if (TextUtils.equals(text, kw)) {
						return true;
					}
				}
			}
		...
		try {
			if (mWakeupKeywords_User != null && mWakeupKeywords_User.length > 0) {
	...
	private void savePcm(String sFileName){
		File tmpFile = new File("/sdcard/alldemo/voice_wk", sFileName.replace("-", "负"));
		try {
	...
	private void onWakeup(String text, long attackTime, float score) {
		JNIHelper.logd("onWakeupXXX text = "+text+" , score = "+score);
		do {
			//主唤醒词直接唤醒
			if (isMainWakeupKw(text)) {
				break;
			}
			//非主唤醒词,大于-4.3分才唤醒词
			if (score > -4.3f){
		...
		// 拦截所有唤醒词, 包括免唤醒词
		{
			JSONBuilder json2 = new JSONBuilder();
			json2.put("keywords", text);
			json2.put("action", "wakeup");
			json2.put("scene", "wakeup");
			...
			if (SenceManager.getInstance().noneedProcSence("wakeup",
					json2.toBytes())) {
				// 双声道回音消除, 需要保存双声道声音
				int aecType = ProjectCfg.getFilterNoiseType();
				if (aecType == 3 || aecType == 1) {
					String strFileName = "txz_" + text + "_" + score + "_"
							+ taskId + "_stereo.pcm";
					savePcm(strFileName);
				}
				return;
		...
		if(isTagWakeup(text)){
			JNIHelper.logd("isTagWakeupKeywords true");
			...
			return;
		}else if(isWakeupKWS(text)){
			ReportUtil.doVoiceReport(new ReportUtil.Report.Builder().setKeywords(text).setAction("wakeup")
			        .setRecordType(UiRecord.RECORD_TYPE_WAKEUP_KW).setTaskID(wakeupId+"").buildWakeupReport(),
		...
		// 根据唤醒词类别判断是否需要进入免唤醒识别
		if(isInstantAsrKeyword(text)){
			mCurrDirectAsrKw = text;
			LaunchManager.getInstance().launchWithInstantAsr(style);
		}else{
...
//	}
	...
	public byte[] getUserWakeupkeywords() {
		...
		StringBuffer sb = new StringBuffer();
		for (int i = 0; i < mWakeupKeywords_User.length; i++) {
			sb.append(mWakeupKeywords_User[i]).append(",");	
		}
		JNIHelper.logd("getUserWakeupKeywords:: " + sb);
