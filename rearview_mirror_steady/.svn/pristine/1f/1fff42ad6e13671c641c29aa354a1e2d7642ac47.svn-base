...
public class FmManager extends IModule{
	...
	private final static String TAG ="FmManager::";
	private final static String DEFAULT_FM_NAMES_FILE_NAME ="res_txz_fm_names.json";
	private final static String DEFAULT_FM_NAMES_PATH =FilePathConstants.BASE_PATH_PRIOR + DEFAULT_FM_NAMES_FILE_NAME;

	private HashMap<String,String> mFMNamesMap;
	...
	public int initialize_BeforeStartJni() {
		...
		regEvent(UiEvent.EVENT_INNER_NET,
				UiInnerNet.SUBEVENT_DOWNLOAD_HTTP_FILE_COMPLETE);
	...
	public int onEvent(int eventId, int subEventId, byte[] data) {
		...
		switch (eventId) {
			...
			case UiEvent.EVENT_INNER_NET: {
				switch (subEventId){
					case UiInnerNet.SUBEVENT_DOWNLOAD_HTTP_FILE_COMPLETE:
	...
	private void onDownloadComplete(byte[] data) {
		...
		try {
			UiInnerNet.DownloadFile downloadFile = UiInnerNet.DownloadFile.parseFrom(data);
			if(TextUtils.equals(DEFAULT_FM_NAMES_FILE_NAME, downloadFile.strName)){
				readFMNames();
			}
	...
	private void toFmFreq(byte[] data){
		...
		try {
			...
			String spk = NativeData.getResPlaceholderString("RS_FM_TO", "%CHANNEL%", fFreq + strUnit);
	...
	private void toAmFreq(byte[] data){
		...
		if (mAmTool == null){
			String spk = NativeData.getResString("RS_AM_NO_TOOL");
			RecorderWin.speakTextWithClose(spk, null);
			return;
		}
		...
		try {
			...
			ReportUtil.doReport(new ReportUtil.Report.Builder().setType("fm").setAction("am")
					.setSessionId().buildCommReport());
	 ...
	 * 读取收音机电台表
	 */
	private void readFMNames() {
		if (enableFmNamesFunc()) {
			LogUtil.logd(TAG + "readFMNames path1 = " + DEFAULT_FM_NAMES_PATH
					+ ", path2 = " + ProjectCfg.getFMNamesPathAdapter());
			mFMNamesMap = new HashMap<String, String>();
			File file = new File(DEFAULT_FM_NAMES_PATH);
			if (!file.exists()) {
				file = new File(ProjectCfg.getFMNamesPathAdapter());
			}
			if (!file.exists()) {
				return;
			}
			LogUtil.logd(TAG + "readFMNames from " + file.getAbsolutePath());
			JNIHelper.sendEvent(UiEvent.EVENT_CUSTOM_FM, FmData.SUBEVENT_FM_NAMES_PATH, file.getAbsolutePath());
			JSONBuilder jsonBuilder = new JSONBuilder(file);
			JSONObject jsonObject = jsonBuilder.getJSONObject();
			Iterator<String> iterator = jsonObject.keys();
			while (iterator.hasNext()) {
				String name = iterator.next();
				String hz = jsonBuilder.getVal(name, String.class);
				mFMNamesMap.put(name, hz);
			}
	 ...
	 * @return
	 */
	public String transFMName2Freq(String fmName){
		if(mFMNamesMap == null || fmName == null){
			return null;
		}
		return mFMNamesMap.get(fmName);
	}

	public boolean enableFmNamesFunc() {
		...
		if (mServerConfig == null
				...
				|| ((mServerConfig.uint64Flags & UiEquipment.SERVER_CONFIG_FLAG_ENABLE_NAME_TO_RADIO) == 0)) {
			LogUtil.logd(TAG + "enableFmFunc false");
			return false;
		}
		LogUtil.logd(TAG + "enableFmFunc true");
		return true;
	}
