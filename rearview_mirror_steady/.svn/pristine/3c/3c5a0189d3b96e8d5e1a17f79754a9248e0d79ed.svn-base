...
public class ChatShockView extends IChatShockView{
	...
	public void init() {
		...
		llContentBg = LayouUtil.getDrawable("white_range_stock_layout");
	...
	public ViewAdapter getView(ViewData data) {
		...
		mLayout.setPadding(0, 0, 0, llContentPaddingBottom);
		...
		vDivider.setBackgroundColor(Color.parseColor("#4CFFFFFF"));
		...
		llStockInfo.setPadding(0, 0, llStockInfoPaddingRight, 0);
...
//		mRelativeLayoutParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
		...
		tvPrice.setText(LanguageConvertor.toLocale(subZeroAndDot(format(price))));
		...
		if (!TextUtils.isEmpty(mStockInfo.strChangeRate)) {
			tvChangeRate.setText(LanguageConvertor.toLocale(addMark(subZeroAndDot(format(mStockInfo.strChangeRate))) + "%"));
...
//					Float.parseFloat(mStockInfo.strYestodayClosePrice)));
		...
		tvYestodayClosePrice.setText(LanguageConvertor.toLocale(format(mStockInfo.strYestodayClosePrice)));

		tvTodayOpenPriceLabel.setText(LanguageConvertor.toLocale(LayouUtil.getString("label_stock_today_open_price")));
		tvTodayOpenPrice.setText(LanguageConvertor.toLocale(format(mStockInfo.strTodayOpenPrice)));
		...
		tvHighestPrice.setText(LanguageConvertor.toLocale(format(mStockInfo.strHighestPrice)));
		...
		tvLowestPrice.setText(LanguageConvertor.toLocale(format(mStockInfo.strLowestPrice)));
