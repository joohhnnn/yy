...
class MyImageSpan extends ImageSpan {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom,
        ...
        // 设置Drawable size
        int drawableSize = fm.bottom - fm.top;
        float scale = drawableSize * 1.0f / drawable.getIntrinsicHeight();
        ...
        if (mVerticalAlignment == ALIGN_BASELINE) {
            ...
            transY = ((y + fm.descent) + (y + fm.ascent)) / 2 - (int)(drawable.getBounds().bottom / 2 * scale);
        }
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
        ...
        int desiredSize = fmi.bottom - fmi.top;
        // setBounds内部有bounds rect是否发生改变的判断, 所以不需要再做额外的处理
        d.setBounds(0, 0, desiredSize, desiredSize);
