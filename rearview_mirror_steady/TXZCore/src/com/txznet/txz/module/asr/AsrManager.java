...
public class AsrManager extends IModule {
	...
	public void onAsrEnd(AsrOption option,
		...
		NewsManager.getInstance().stop();//触发过识别的话，需要需要取消新闻。之所以不在start处取消新闻，是因为随意打断功能中，播报TTS的过程中，会启动识别。AndyZhao
...
//							//在这里判断TTS是否播报完了，播报完成就不需要打断
	...
	public byte[] invokeCommAsr(final String packageName, String command,
		...
		else if(command.equals("comm.asr.clearAsrListene")){
			asrListener = null;
			return null;
		}
		else if(command.equals("comm.asr.setAsrListener")){
			asrListener = packageName;
			return null;
		}
