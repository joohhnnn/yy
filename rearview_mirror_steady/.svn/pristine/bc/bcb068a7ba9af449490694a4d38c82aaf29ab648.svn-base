...
//    }
    ...
    private String getUrlHashCode() {
        //"请告知后台，这里需要带上&ttth，协商为不同的路径，相同的歌曲，否则客户端，将不会落地文件"
        String flag = "&ttth=";
        int index = mPlayItem.getUrls().get(mUrlIndex).indexOf(flag);
        if (index < 0) {
            flag = "?ttth=";
            index = mPlayItem.getUrls().get(mUrlIndex).indexOf(flag);
            if (index < 0) {
                Logger.e("music:back:", new RuntimeException("请告知后台，这里需要带上ttth，协商为不同的路径，相同的歌曲，否则客户端，将不会落地文件"));
            }
        }
