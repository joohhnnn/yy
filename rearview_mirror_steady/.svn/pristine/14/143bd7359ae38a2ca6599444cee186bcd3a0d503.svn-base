...
public class ProjectCfg {
	...
	//离线保存录音文件广播权限
	public static final String BROADCAST_PERMISSION_SAVE_DATA = "com.com.txznet.txz.module.asr.offlineSaveData";

	//离线保存录音文件标志位SAVE_DATA，默认false
	private static  boolean sOfflineAsrSaveData = false;
	...
	public static boolean getOfflineAsrSaveData() {
		return sOfflineAsrSaveData;
	}

	public static void setOfflineAsrSaveData(boolean bSaveData) {
		sOfflineAsrSaveData = bSaveData;
		LogUtil.logd("setOfflineAsrSaveData bSaveData = " + bSaveData);
	}
...
//	static String mIflyAppId = "548a6747";
	...
	public static void setBaiduAppid(String strAppid){
		ProjectCfg.mBaiduAppid = strAppid;
	}
	
	public static void setBaiduApikey(String strApiKey){ 
		ProjectCfg.mBaiduAK = strApiKey;
	}
	
	public static void setBaiduSecret(String strApiSecret){
		ProjectCfg.mBaiduSK = strApiSecret;
	}
	...
	public static String getBaiduSK(){
		return mBaiduSK;
	}
	
	static String mBaiduAppid;
	static String mBaiduAK;
	static String mBaiduSK;
	...
	static String sFangdeToken = PreferenceUtil.getInstance().getString(PreferenceUtil.KEY_FANGDE_TOKEN, "");
	
	public static String getFangdeToken() {
		return sFangdeToken;
	}

	public static void setFangdeToken(String token) {
		sFangdeToken = token;
	}
	/**
	 ...
	 * 外部设置的录音缓冲区大小
	 */
	private static Integer sRecorderBufferSize = null;
	public static void setRecorderBufferSize(final Integer recorderBufferSize) {
		sRecorderBufferSize = recorderBufferSize;
	}

	public static Integer getRecorderBufferSize() {
		return sRecorderBufferSize;
	}
...
//		}
	...
	public static final int YZS_SDK_VERSION = 0;//云知声重要更新版本号, 0表示忽略版本号信息
	...
	/** 唤醒生成缓存唤醒词语法文件，bool值，默认为false */
	private static boolean sWakeupPrebuiltGrammar = false;
	
	/**
	 * @return 唤醒生成缓存唤醒词语法文件
	 */
	public static boolean wakeupPrebuiltGrammar() {
		boolean bret = false; 
		int yzs_sdk_ver = YZS_SDK_VERSION;
		bret = sWakeupPrebuiltGrammar && (3 == yzs_sdk_ver);//V3引擎版本才支持唤醒词预编译优化模式
		return bret;
	}
	...
	private static int sMemMode = MEM_MODE_NONE;
	
	private final static int[] sMemModeListSupported = {MEM_MODE_NONE};
	public static void setMemMode(int mode){
		if (sMemModeListSupported == null){
			return;
		}
		//版本支持的模式，才允许外部设置，
		//否则后续语音升级到不支持该模式的版本时,会出现紊乱。
		boolean bSupported = false;
		for (int value : sMemModeListSupported){
			if (value == mode){
				bSupported = true;
				break;
			}
		...
		sMemMode = mode;
		sWakeupPrebuiltGrammar = sMemMode == MEM_MODE_PREBUILD || sMemMode == MEM_MODE_PREBUILD_MERGE;
		LogUtil.logd("setMemMode:" + sMemMode + ",sWakeupPrebuiltGrammar:" + sWakeupPrebuiltGrammar);
	}

	public static int getMemMode(){
		return sMemMode;
	}


	//判断是否关闭在线识别
	private static UiEquipment.VoiceConfig sVoiceConfig ;

	/**
	 * 语音的配置，目前是只有关闭在线引擎，预留了关闭离线引擎
	 * @param sVoiceConfig
	 */
	public static void setVoiceConfig(UiEquipment.VoiceConfig sVoiceConfig) {
		ProjectCfg.sVoiceConfig = sVoiceConfig;
	}

	public static boolean isDisableNetAsr(){
		return (sVoiceConfig != null && sVoiceConfig.bOnlineDisable != null && sVoiceConfig.bOnlineDisable);
	}

	public static String getDisableNetAsrTts(){
		String tts = null;
		if (sVoiceConfig.onlineDisableTts != null && sVoiceConfig.onlineDisableTts.length > 0) {
			tts = new String(sVoiceConfig.onlineDisableTts);
		}
		return tts;
	}
	...
	//由适配控制的是否开启打字效果
	private static Boolean bSDKTypingEffect = null;

	public static void setSDKTypingEffect(boolean bSDKTypingEffect) {
		ProjectCfg.bSDKTypingEffect = bSDKTypingEffect;
		UserConf.getInstance().getFactoryConfigData().mEnableTypingEffectItem = isSupportTypingEffect();
		UserConf.getInstance().saveFactoryConfigData();
	}

	public static boolean isSDKTypingEffect() {
		return bSDKTypingEffect == null || bSDKTypingEffect;
	}

	//开启打字效果
	public static boolean enableTypingEffect() {
		if (DebugCfg.TYPING_EFFECT_DEBUG) {
			return true;
		}
		boolean ret = false;
		if (isSupportTypingEffect()) {//判断是否支持
			ConfigData configData = UserConf.getInstance().getUserConfigData();
			if (configData.mUseTypingEffect == null || configData.mUseTypingEffect){ //判断用户是否开启
				ret = true;
			}
		}
		return ret;
	 ...
	 * @return
	 */
	public static boolean isSupportTypingEffect() {
		boolean ret = true;
		do {
			//界面相关支持的判断
			if (WinManager.getInstance().hasThirdImpl()) {
				if (bSDKTypingEffect == null || !bSDKTypingEffect ) {
					//第三方界面，并且没有适配打字效果
					ret = false;
					break;
				}
			} else if (WinManager.getInstance().isRecordWin2()) {
				if (!com.txznet.comm.ui.util.ConfigUtil.useTypingEffect()) {
					//皮肤包，并且没有适配打字效果
					ret = false;
					break;
				}
			...
			if (!isSDKTypingEffect()) {//判断方案商是否开启
				ret = false;
				break;
			}
		}while (false);
	...
	public static boolean isSupportHomeControl() {
		UiEquipment.ServerConfig serverConfig = ConfigManager.getInstance().getServerConfig();
		return serverConfig != null && serverConfig.uint64Flags != null && (serverConfig.uint64Flags & UiEquipment.SERVER_CONFIG_FLAG_ENABLE_HOME_CONTROL) != 0;
	}

	public static boolean isSupportCorrectSpeech() {
		boolean result;
		UiEquipment.ServerConfig serverConfig = ConfigManager.getInstance().getServerConfig();
		result =  serverConfig != null && serverConfig.uint64Flags != null && (serverConfig.uint64Flags & 67108864) != 0;
		return !result;
	}
    
   //降噪会影响在线识别率, 所以使用了地平线等引擎的降噪后，识别需要使用预处理之前的数据
   //默认使用预处理之后的数据
    private static boolean sUseSePreprocessedData = true;
    
    public static boolean isUseSePreprocessedData(){
    	return sUseSePreprocessedData;
    }
    
    public static void setUseSePreprocessedData(boolean bUsed){
    	sUseSePreprocessedData = bUsed;
    }


    //是否默认使用2.0的皮肤包
	public static final Boolean DEFAULT_USE_2_0 = true;

    //适配设进来的收音机电台表路径
    private static String sFMNamesPathAdapter = "";

    public static String getFMNamesPathAdapter(){
    	return sFMNamesPathAdapter;
	}

	public static void setFMNamesPathAdapter(String path){
		sFMNamesPathAdapter = path;
	}
