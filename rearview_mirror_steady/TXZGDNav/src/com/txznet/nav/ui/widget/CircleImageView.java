...
public class CircleImageView extends ImageView{
	...
	private Paint mCovePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	
	private void buildCoverBitmap(){
		...
		mCoverBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Config.ARGB_8888);
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isDrawCover){
			canvas.drawBitmap(mCoverBitmap, 0 , 3 , mCovePaint);
			canvas.drawCircle(getWidth() / 2, getHeight() / 2, mBorderRadius - 8, mBgPaint);
			canvas.drawCircle(getWidth() / 2, getHeight() / 2, mBitmapRadius - 8, mBitmapPaint);
			canvas.drawCircle(getWidth() / 2, getHeight() / 2, mBorderRadius - 8, mBorderPaint);
		}else {
	...
	public void setDrawCover(boolean isDraw){
		this.isDrawCover = isDraw;
		invalidate();
	}
