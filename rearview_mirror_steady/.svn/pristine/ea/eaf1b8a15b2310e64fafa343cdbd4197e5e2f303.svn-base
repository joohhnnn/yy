 ...
 * 无屏模式采用的布局包装类
 */
public class WinLayoutNoneWrapper extends IWinLayout {

	private static WinLayoutNoneWrapper sInstance = new WinLayoutNoneWrapper();

	private WinLayoutNoneWrapper() {
	}

	public static WinLayoutNoneWrapper getInstance() {
		return sInstance;
	}
	...
	public void init() {
		...
		if (mRootLayout == null) {
			...
			if (WinLayout.isVertScreen){
                ...
                contentHeight = (int) LayouUtil.getDimen("x375");
                talkHeight = (int) LayouUtil.getDimen("x103");
                talkMagin = (int) LayouUtil.getDimen("x20");    //对话框左右边距
                intervalHeight =(int) LayouUtil.getDimen("x10");   //对话框与内容框间隔
                paddingRight = (int) LayouUtil.getDimen("x10");   //到屏幕边缘的横向间隔
                paddingBottom = (int) LayouUtil.getDimen("x5");    //到屏幕边缘的纵向间隔
                helpIconSzie = (int) LayouUtil.getDimen("x51");
            }else {
...
//			llParams.topMargin = (int) LayouUtil.getDimen("x10");
			...
			llParams = (LayoutParams) rlRecordView
					.getLayoutParams();
			...
			flTalk = (FrameLayout) LayouUtil.findViewByName("flTalk",
					tempLayout);
			llParams = (LayoutParams) flTalk.getLayoutParams();
			...
			llParams = (LayoutParams) llHelp.getLayoutParams();
			llParams.width = talkHeight;
			llParams.height = talkHeight;
			...
			llContent.setBackground(LayouUtil
					.getDrawable("bg_none"));
			llParams = (LayoutParams) llContent
					.getLayoutParams();
...
//		int h = mRootLayout.getMeasuredHeight();
	...
	private void changeLayoutBaseLocation() {
		if (historyX < screenWidth / 2) {
			...
			// llHelp.setPadding((int) LayouUtil.getDimen("x20"), 0,
			// (int) LayouUtil.getDimen("x20"), 0);
			llHelp.setBackground(LayouUtil
					.getDrawable("help_bg_right"));
			llParams = new LayoutParams(talkHeight,talkHeight);
			llParams.gravity = Gravity.CENTER;
			/*llParams.leftMargin = dimen_x10;*/
			// llParams.rightMargin = (int) LayouUtil.getDimen("x20");
		...
		if (historyY < screenHeight / 2) {
			...
			llParams = new LayoutParams(
					LayoutParams.WRAP_CONTENT,talkHeight);
			...
			llParams = new LayoutParams(contentWidth,contentHeight);
			llParams.gravity = hor_gravity;
