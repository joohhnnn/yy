...
//		mCacheWkWords = cmds;
	...
	private static final String DEFAULT_WAKEUP_TMP_PATH = GlobalContext.get().getApplicationInfo().dataDir + "/" + PRIV_FILESDIR_NAME + "/YunZhiSheng/asrfix/wakeup.dat.tmp";
	...
	private boolean saveWakeupKeywords(CmdCompileTask task) {
		// 保存唤醒词语法
		if (FileUtil.copyFile(DEFAULT_WAKEUP_PATH, DEFAULT_WAKEUP_TMP_PATH)) {
			if (checkWakeupGrammarFile(DEFAULT_WAKEUP_PATH, DEFAULT_WAKEUP_TMP_PATH)) {
				File file = new File(DEFAULT_WAKEUP_TMP_PATH);
				String destDir = genKwsGrammar(task);
				if (!file.renameTo(new File(destDir))) {
					LogUtil.loge("renameTo destDir error, update keywords : " + task.getTaskId());
				}
				LogUtil.logd("update keywords : " + task.getTaskId());
				//mWakeupKeywordsMap.put(task.getTaskId(), task.getKws());
			} else {
				LogUtil.loge("grammar md5 check error, update keywords : " + task.getTaskId());
			}
		}
		return true;
	}
	
	private void onCompileWakeupWordDone() {
		if (mCurrentCompileTask == null) {
			return;
		}
		saveWakeupKeywords(mCurrentCompileTask);
		//非预编译的唤醒词需要重新启动唤醒引擎才能生效
		//
		//if (!WakeupCmdTask.isPreBuildType(mCurrentCompileTask.getTaskKwsType())) 
		{
	...
	private synchronized void execCompileTask() {
	...
	private static boolean checkWakeupGrammarFile(String src, String dst){
		boolean bRet = false;
		do {
			...
			try {
				String srcMD5 = MD5Util.generateMD5(new File(src));
				String dstMd5 = MD5Util.generateMD5(new File(dst));
				if (TextUtils.isEmpty(srcMD5) || TextUtils.isEmpty(dstMd5)) {
					break;
				}
				bRet = TextUtils.equals(srcMD5,dstMd5);
			} catch (Exception e) {
				LogUtil.logd("checkWakeupGrammarFile error : " + e.getMessage());
			}
		}while (false);
