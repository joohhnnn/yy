...
public class BaseBarPresenter implements BaseBarContract.Presenter, IPlayListChangedListener, PlayerInfoUpdateListener, Observer {
    ...
    public void onPlayInfoUpdated(Audio audio, Album album) {
        ...
        if (Utils.isSong(audio.getSid())) {//该功能问过产品，根据当前播放的音频的来源判断是显示收藏按钮还是显示订阅按钮，来源音乐的则显示收藏按钮，来源电台的只能显示订阅按钮
            ...
            album = PlayInfoManager.getInstance().getSubscribeAlbum();
            if (album != null) {
    ...
    public void showEmptyInfo() {
        mView.onPlayInfoUpdated(null, null);
    }
