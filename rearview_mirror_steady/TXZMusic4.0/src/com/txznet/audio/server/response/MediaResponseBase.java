...
public abstract class MediaResponseBase extends Response {
                ...
                if (false) {
                    if (fosFileOutputStream == null) {
                        ...
                        try {
                            ...
                            fosFileOutputStream = null;
                            LogUtil.loge("(Audio)(MediaResponse)FileOutputStream:"
    ...
    /**
     * @return 负数则为时长 正数代表数据长度
     */
    public long calDataOrTime() {
        return mSess.player.getDataPieceSize();
    }

    protected boolean waitNeedData(OutputStream out) throws IOException {
        long reserveData = 0;/* 预缓冲至少要保留的数据量 */
        long writeTotalData = 0;/* 播放器整体的数据量 */
        float readPlayedData = 0;/* 当前播放过得数据量 */
        long data = 0;// 30s数据量长度
        ...
        while (true) {
            ...
            try {
                ...
                if (!ret) {
                    ...
                    writeTotalData = (mWrite + mFrom);
                    writePercent = writeTotalData * 1f / mLen;

                    if (writePercent - mSess.player.getPlayPercent() < 0.15) {
                        LogUtil.logw("media session[" + mSess.getLogId()
                                + "] need more data warning");
                        ret = true;
                    }

                    if ((data = mSess.player.getDataPieceSize()) < 0) {// 表示时长
                        data = mLen / mSess.player.getDataPieceSize() * -1 * TXZAudioPlayer.PREPARE_BUFFER_DATA_TIME;
                        mSess.player.settDataPieceSize(data);
                    }
                    reserveData = (data * TXZAudioPlayer.NEED_BUFFER_DATA_TIME / TXZAudioPlayer.PREPARE_BUFFER_DATA_TIME);
                    readPlayedData = mLen * mSess.player.getPlayPercent();
                    //TODO:限流去掉，返回全部数据
//                    reserveData = mLen;//(data * TXZAudioPlayer.NEED_BUFFER_DATA_TIME / TXZAudioPlayer.PREPARE_BUFFER_DATA_TIME);
//                    readPlayedData = mLen * mSess.player.getPlayPercent();

//					if (Constant.ISNEED) {
//						LogUtil.logd(TAG + " waitNeedData :param1=" +writeTotalData
//								+ ",param2=" + mSess.player.getDataPieceSize() + ",size=" + reserveData
...
//					}
    ...
    protected MediaResponseBase(Socket out, SessionInfo sess, long from, long to) {
        ...
        mTo = to;
        mSocketOut = out;
        LogUtil.logd(BlockTcpClient.TAG + "socket:create," + mSocketOut.hashCode());
        mWrite = 0;
    ...
    public void cancel() {
        ...
        if (mSocketOut != null) {
            try {
                LogUtil.logd(BlockTcpClient.TAG + "socket:close," + mSocketOut.hashCode());
                mSocketOut.close();
            } catch (Exception e) {
    ...
    public void print(OutputStream out) throws IOException {
        ...
        mSess.len = mLen;
        if (mLen < 0) {
            LogUtil.loge(TAG + "get length is " + mLen, new RuntimeException());
            return;
        }
        printEmptyData(out);
