...
//		mRecognizer.loadCompiledJsgf("txzTag", GlobalContext.get().getApplicationInfo().dataDir + "/data/txz.dat");// 加载编译语法
	...
	private void loadAsrGrammar(){
		String grammarPath = GlobalContext.get().getApplicationInfo().dataDir + "/data/txzTag.dat";// 加载上一次生成的模型
	...
	public void setWakeupKeywords(String[] keywords) {
		...
		keywords = new String[] { "切换下一首", "切换下一曲", "切换上一首", "切换上一曲", "播放音乐", "开始播放", "暂停音乐", "暂停播放", "关闭屏幕", "打开屏幕", "你好小踢", "语音助手", 
				"确定", "取消", "第一个", "第二个", "第三个", "第四个", "上一页", "下一页" };
	...
	private void initCompiler() {
		...
		if (bCompilerInited) {
			mRecognizer.setOption(SpeechConstants.ASR_OPT_SET_COMPILE_MAX_PRONUNCIATION,
					COMPILE_MAX_PRONUNCIATION);// 设置最大发音长度
