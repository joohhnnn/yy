...
public class AudioListView extends IAudioListView {
	...
	private boolean isHasTag(AudioListViewData audioListViewData){
        for (int i = 0;i < audioListViewData.count;i++){
            ...
            if (audioBean.novelStatus != AudioBean.NOVEL_STATUS_INVALID ||
                    ...
                    audioBean.lastPlay ||
                    audioBean.latest){
                LogUtil.logd(WinLayout.logTag+ "true--AudioBean: "+audioBean.paid+"--"+audioBean.lastPlay+"--"+audioBean.latest+"--"+audioBean.novelStatus);
                return true;
            }
	...
	private void initFull(){
		if (WinLayout.isVertScreen){
            ...
            ivTagSide = (int) LayouUtil.getDimen("musicTagSide");
            ivTagMargin = unit/3;
		}else {
	...
	private void initNone(){
        if (WinLayout.isVertScreen){
            ...
            tvDescHeight = (int) LayouUtil.getDimen("vertical_h6_height");
            ivTagSide = (int) LayouUtil.getDimen("musicTagSide");
            ivTagMargin = (int) LayouUtil.getDimen("x3");
        }else {
            int unit = (int) LayouUtil.getDimen("unit");
            tvNumSide =  6 * unit;
            tvNumHorMargin =  unit;
