...
public class UdpDataFactory {
	...
	public static class UdpData {
	    ...
	    public static boolean isValidCmd(int cmd) {
	    	...
	    	switch (cmd){
				...
				case CMD_RESP_CONNECTION:
				case CMD_LOG:
				case CMD_LOG_ACK:
				case CMD_REPORT:
				case CMD_REPORT_IMME:
					isValidCmd = true;
					break;
			}
			return isValidCmd;
     ...
     * @param cmd
     *              对方执行哪个方法 打日志{@link UdpData#CMD_LOG} 上报数据{@link UdpData#CMD_REPORT}
     * @param originalData
     *              原始数据
     * @return
     */
    ...
    /**
     * 从传输数据解析出原始数据，跟{@link #getTransferData(int, int, int, byte[])}相对应
     * @param transferData
     * @return
     */
...
//            throw new RuntimeException("transfer data can't be shorter than reserve length!");
		...
		if (udpData.udpId < 0 || (transferData[1] != 0 && transferData[1] != 1)){
			return null;
		}
		...
		if (udpData.length <= 0
				|| udpData.length > UdpConfiger.getInstance().getUserDataLength()) {
//			udpData.length = 0;
//			udpData.data = new byte[0];
//			return udpData;
			return null;
		}
