...
public class UIData {
	public static byte[] getUIData(int dataId, byte[] param) {
		...
		case com.txz.ui.data.UiData.DATA_ID_SYSTEM_ENV: {
			...
			env.strHardwareParams = DeviceInfo.getHardwareParams();
		...
		case UiData.DATA_ID_REPORT_DATA:
				UiData.ReportData reportData = new UiData.ReportData();
				UiEquipment.LocalAsrEngine localAsrEngine = new UiEquipment.LocalAsrEngine();
				localAsrEngine.uitn32EngineType = AsrLocalEngineController.getInstance().getUsedType();
				reportData.msgLocalAsrEngine = localAsrEngine;
				UiEquipment.PreProcessEngine aecEngine = new UiEquipment.PreProcessEngine();
				aecEngine.uint32EngineType = VoicePreProcessorController.getInstance().getUsedAECType();
				reportData.msgPreProcessAecEngine = aecEngine;
				UiEquipment.PreProcessEngine seEngine = new UiEquipment.PreProcessEngine();
				seEngine.uint32EngineType = VoicePreProcessorController.getInstance().getUsedSEType();
				reportData.msgPreProcessSeEngine = seEngine;
				return MessageNano.toByteArray(reportData);
			case UiData.DATA_ID_GET_MERGE_CMD_PATH:
				if (CommandManager.getAdapterLocalCommandLoadPath() != null) {
					return CommandManager.getAdapterLocalCommandLoadPath().getBytes();
				} else {
					return null;
				}
		}
