	...
	private byte[] invokeCommTts(final String packageName, String command, byte[] data) {
		try {
			if (command.equals("comm.tts.speak")) {
				...
				int remoteTtsId = TtsManager.getInstance().speakVoice(iStream, sText, voiceUrls, delay, bPreempt, true,
						new ITtsCallback() {
							@Override
...
//					}
					...
					if (forceUseUI1 != null) {
						if (forceUseUI1) {
							WinManager.getInstance().forceUseUI1();
						}
					}
					// 获取license，并初始化声控引擎
					if (appId != null && appToken != null) {
