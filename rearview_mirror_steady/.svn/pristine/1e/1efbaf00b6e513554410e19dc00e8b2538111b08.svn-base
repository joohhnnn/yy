...
import com.txznet.comm.ui.dialog2.WinNotice.WinNoticeBuildData;
	...
	public static int checkSDCardError() {
		if (checkFileReadOnly()) {
			...
			if (pathAvailable != null && pathAvailable.size() != 0) {
				WinNoticeBuildData buildData = new WinNoticeBuildData();
				buildData.setMessageText("磁盘写入出现问题，请重启应用!", true);
				// 弹出提示框
				new WinNotice(buildData,true) {
