...
public class WakeupManager extends IModule {
	...
	private String mInterruptKw = "停止播报";
	...
	private WakeupManager() {
		...
		updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_USER, mWakeupKeywords_User);
		updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_USER, true);
...
//		updateProtectKeywords(kws);
	...
	private String[] getInstantWakeupKwsWithNick() {
		...
		updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_ONESHOT, ret);
		updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_ONESHOT, true);
	...
	public void updateWakuepKeywords_Normal(String[] cmds){
		...
		synchronized (WakeupManager.class) {
			...
			updateOneShotKeyWords();
			updateWakeupKeywords2Asr(mWakeupKeywords_Sdk);
			updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_SDK, mWakeupKeywords_Sdk);
			updateKeywordByTaskId(WakeupCmdTask.WAKEUP_TASK_ID_SDK, true);
			++updateKeywordsCur;
...
//		}
	...
	public void useWakeupAsAsr(String service, final IWakeupAsrCallback callback) {
		...
		synchronized (WakeupManager.class) {
			...
			updateKeywordByTaskId(service + "@" + callback.getTaskId(), callback.genKeywords());
			updateKeywordByTaskId(service + "@" + callback.getTaskId(), true);
		...
		synchronized (mWakeupKeywords_Asr) {
			...
			for (int i = 0; i < mWakeupKeywords_Asr.size();) {
				...
				if (oldTask.service.equals(service)
					...
					if (oldTask != null){
						final IWakeupAsrCallback cb = oldTask.callback;
						if (cb != null){
							updateKeywordByTaskId(service + "@" + cb.getTaskId(), false);
						}
					}
...
//	}
	...
	private String[] getWakeupKeywords() {
		if (!ProjectCfg.wakeupPrebuiltGrammar()) {
			return keyWords;
		}
		return WakeupCmdTask.genWakeupKwsFromTasks(mWakeupKeywordMap);
	}
	
	boolean bExclusiveCmdsPreBuild = false;
	private void preBuildCommKws(String[] cmds){
		if (!ProjectCfg.wakeupPrebuiltGrammar()) {
			return;
		}
		//初始化未成功设置唤醒词无效
		if (!isInitSuccessed()){
			LogUtil.logw("preBuildCommKws bInitSuccessed:" + isInitSuccessed());
			return;
		}
		
		//提前编译需要快速响应的Exclusive类型唤醒词
		//只需要尝试编译一次
		if (!bExclusiveCmdsPreBuild){
			LogUtil.logd("preBuildCommKws exclusive kws");
			bExclusiveCmdsPreBuild = true;
			String[][] preBuildExclusiveCmds = {WakeupCmdTask.INCOMING_TASK_KWS, WakeupCmdTask.RECORDING_TASK_KWS};
			for (int i = 0; i < preBuildExclusiveCmds.length; ++i){
				String[] kws = WakeupCmdTask.genKws(WakeupCmdTask.TYPE_EXCLUSIVE_MASK, preBuildExclusiveCmds[i]);
				if (kws != null){
					mWakeup.setWakeupKeywords(kws);
				}
			}
		...
		//提前编译需要快速响应的static类型唤醒词
		LogUtil.logd("preBuildCommKws static kws");
		String[][] preBuildStaticCmds = {WakeupCmdTask.STATIC_POI_SELECT_CMDS, WakeupCmdTask.STATIC_DEFAULT_SELECT_CMDS};
		for (int i = 0; i < preBuildStaticCmds.length; ++i)
		{
			String[] kws = WakeupCmdTask.mergeKws(cmds, preBuildStaticCmds[i]);
			if (kws != null){
				mWakeup.setWakeupKeywords(kws);
			}
		}
	...
	private Map<String, WakeupCmdTask> mWakeupKeywordMap = new HashMap<String, WakeupCmdTask>();
	/**
	 * 根据唤醒词类型更新唤醒词，输入唤醒词可以不为全量，需要在更新出按需求更新
	 * @param keywordType
	 * @param keywords
	 */
	private void updateKeywordByTaskId(String strTaskId, String[] keywords) {
		if (!ProjectCfg.wakeupPrebuiltGrammar()) {
			return;
		}
		WakeupCmdTask.updateWakeupCmdTaskMap(mWakeupKeywordMap, strTaskId, keywords);
	}
	
	private void updateKeywordByTaskId(String strTaskId, boolean bUsed) {
		if (!ProjectCfg.wakeupPrebuiltGrammar()) {
			return;
		}
		WakeupCmdTask.updateWakeupCmdTaskMap(mWakeupKeywordMap, strTaskId, bUsed);
	}
}
