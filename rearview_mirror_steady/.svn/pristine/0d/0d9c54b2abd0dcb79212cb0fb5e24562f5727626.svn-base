...
public class WxStatusHelper {
    ...
    public void notifyLoginUserInfoChanged() {
        boolean isLoggedIn = WxLoginStore.get().isLogin();
        ...
        if (isLoggedIn) {
            nickName = WxContactStore.getInstance().getLoginUser().getDisplayName();
            avatar = WxQrCodeStore.get().getScannerPicStr();
        }
