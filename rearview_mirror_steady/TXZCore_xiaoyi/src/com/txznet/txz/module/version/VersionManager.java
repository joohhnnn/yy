			...
			public void run() {
				...
				while (mInstallThread != null) {
					...
					do {
						...
						boolean isWinHelpShow = WinHelpTops.getInstance().isShowing();
						
						if (bSilentUpdate
								&& !isNav && !isRecordWinShow && !isWinHelpShow) // 界面打开中，不允许静默升级
						{
							JNIHelper
						...
						if (MusicManager.getInstance().isPlaying()) // 音乐中
						{
							JNIHelper
...
//											0);
					...
					if (mP1.boolForce != null && mP1.boolForce == true) {
						...
						}.setTitle("软件更新").setMessage(mP1.strDesc)
